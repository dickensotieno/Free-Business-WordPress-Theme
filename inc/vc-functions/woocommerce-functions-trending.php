...
function rd_add_cart_button()
{
	...
	{
		$output = '<a class="add_to_cart_button button product_type_variable" href="'.get_permalink($product->id).'"> '.__("Select options","jadonai").'</a>';
	}

	if(in_array($product->product_type, array('subscription', 'simple', 'bundle')))
	{
		$output .= '<a class="button show_details_button" href="'.get_permalink($product->id).'">  '.__("Show Details","jadonai").'</a>';
	}
	if (!$product->is_in_stock()) {
		
	$output =  '<a href="'.get_permalink().'" rel="nofollow" class="button add_to_cart_button more_info_button out_stock_button"> '.__("Out of Stock","jadonai").'</a>';
	}
	else
	{
...
function rd_woocommerce_header_dropdown_cart_fragment( $fragments ) {
	...
	ob_start();?>
  <div class="current_item_number"><?php echo sprintf(_n('%d', '%d', $woocommerce->cart->cart_contents_count, 'jadonai'), $woocommerce->cart->cart_contents_count);?></div>
...
if(!function_exists('rd_woocommerce_frontend_search_params'))
{
	...
	function rd_woocommerce_frontend_search_params()
	{
		...
		global $rd_config;
		global $rd_data;
		$product_order['default'] 	= __("Default Order",'jadonai');
		$product_order['title'] 	= __("Name",'jadonai');
		$product_order['price'] 	= __("Price",'jadonai');
		$product_order['date'] 		= __("Date",'jadonai');
		$product_order['popularity'] = __("Popularity",'jadonai');

		$product_sort['asc'] 		= __("Click to order products ascending",  'jadonai');
		$product_sort['desc'] 		= __("Click to order products descending",  'jadonai');

		$per_page_string 		 	= __("Products per page",'jadonai');
		...
		$output .= "    	<li><span class='current_li'>".__("Sort by:",'jadonai')." <strong>".$product_order[$po_key]."</strong></span>";
		$output .= "    	<ul>";
		...
		$output .= "    	<li><span class='current_li'>".__("Show:",'jadonai')." <strong>".$pc_key." ".$per_page_string."</strong></span>";
		$output .= "    	<ul>";
...
function rd_woo_custom_product_searchform( $form ) {
	...
	$form = '<form role="search" method="get" id="searchform" action="' . esc_url( home_url( '/'  ) ) . '">
		<div>
			<label class="screen-reader-text" for="s">' . __( 'Search for:', 'jadonai' ) . '</label>
			<input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="' . __( 'Search for products', 'jadonai' ) . '" />
...
<div class="single_product_navigation"><?php $next = get_permalink(get_adjacent_post(false,'',true)); if ($next != get_permalink()) { ?><a href="<?php echo esc_url($next); ?>" class="previous_product"><?php echo __('Previous', 'jadonai'); ?></a><?php } ?><?php $prev = get_permalink(get_adjacent_post(false,'',false)); if ($prev != get_permalink()) { ?><a href="<?php echo esc_url($prev); ?>" class="next_product"><?php echo __('Next', 'jadonai'); ?></a><?php } ?></div>
